...
/** OptionParser mixin that causes the OptionParser to not call exit (call `sys.exit`) if the `--help` option is
  * passed
  */
trait DoNotTerminateOnExit extends OptionParser[Any] {// this: OptionParser[DoNotTerminateOnExit] =>
...
/** OptionParser mixin that converts to [[OptionsException]]
  ...
  */
trait ExceptOnError[T] extends OptionParser[T] {// this: OptionParser[ExceptOnError | Any] =>
